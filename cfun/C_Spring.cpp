...
void solve()
{
    ...
    ll lcm_abc=safe_lcm(safe_lcm(a,b),c);
    ll n_abc=m/lcm_abc;

    auto gw=[&](ll me, ll o1, ll o2)
    {
        ll solo=m/me;
        ll s1=m/safe_lcm(o1,me);
        ll s2=m/safe_lcm(o2,me);
        return 6*solo - 3*(s1+s2) + 2*(n_abc);
    };
    cout<<gw(a,b,c)<<" "<<gw(b,a,c)<<" "<<gw(c,a,b);
}
