...
int rec(vector<int> v, int idx, int ans)
{
    if(idx+1==v.size()) return ans;
    int d1=abs(v[idx]-v[idx+1]);
    int d2=0;
    if(idx+2<v.size())
    d2=abs(v[idx]-v[idx+2]);
    if(d1<d2) {
        ans+=d1;
        return rec(v,idx+1,ans);
    }
    else{
        ans+=d2;
        return rec(v,idx+2,ans);
    }
}
