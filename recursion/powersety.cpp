...
void rec(vector<int> v, int idx, vector<int> take, vector<int> nottake)
{
    if(idx==v.size())
    {
        if(take.size())
        ...
        // cout<<"\n";
        if(nottake.size())
        for(auto& i: nottake) cout<<i<<" ";
        if(take.empty()&&nottake.empty()) cout<<"empty";
        cout<<"\n";
        return;
    }
    rec(v,idx+1,take,nottake);
    take.pb(v[idx]);
    rec(v,idx+1,take,nottake);
}
