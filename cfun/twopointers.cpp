...
void solve() {
    ...
    int l = 0, r = n - 1;
    long long ans = 0; // Use long long to avoid overflow in multiplication

    while (l < r) {
        int add = v[l] + v[r];
        
        if (add == target) {
            // CASE: Both pointers point to the same value
            if (v[l] == v[r]) {
                long long count = r - l + 1;
                ans += (count * (count - 1)) / 2;
                break; // All pairs within this range are now counted
            }

            // CASE: Pointers point to different values
            int left_val = v[l], cnt1 = 0;
            while (l < r && v[l] == left_val) {
                cnt1++;
                l++;
            }
            
            int right_val = v[r], cnt2 = 0;
            while (r >= l && v[r] == right_val) {
                cnt2++;
                r--;
            }
            ans += (1LL * cnt1 * cnt2);
